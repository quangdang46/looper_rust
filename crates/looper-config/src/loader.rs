...
impl ConfigLoader {
    ...
    fn load_file_partial(&self) -> Result<PartialConfig, ConfigError> {
        let path = match &self.config_path {
            ...
            None => discover_config_file().ok_or_else(|| {
                ConfigError::FileNotFound(
                    "no config file found in search paths".into(),
                )
            })?,
        };
        ...
        parse_config_content(&content, &ext).map_err(|detail| ConfigError::ParseError {
            path: path.display().to_string(),
            detail,
        })
    }
...
mod tests {
    ...
    fn test_loader_with_cli_overrides() {
        ...
        let server = crate::partial::PartialServerConfig {
