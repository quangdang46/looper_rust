...
use rusqlite::params;
use grove_types::{
    BeadId, BulletId, RunId, Timestamp,
    playbook::{
...
impl Database {
    ...
    pub fn insert_playbook_bullet(&mut self, bullet: &PlaybookBulletRecord) -> Result<()> {
        self.connection().execute(
            ...
            params![
                ...
                bullet.replaced_by.as_ref().map(|id| id.as_str()),
        ...
        self.with_tx(|tx| {
            tx.execute(
                ...
                params![
                    ...
                    feedback.bead_id.as_ref().map(|id| id.as_str()),
                    feedback.run_id.as_ref().map(|id| id.as_str()),
            ...
            // Update counts
            let (helpful_col, harmful_col) = match feedback.kind {
                FeedbackKind::Helpful => ("helpful_count = helpful_count + 1", "harmful_count"),
                FeedbackKind::Harmful => ("helpful_count", "harmful_count = harmful_count + 1"),
            };
            tx.execute(
                &format!(
                    "UPDATE playbook_bullets SET {helpful_col}, updated_at = ?1 WHERE id = ?2"
                ),
    ...
    pub fn list_active_bullets(&self, scope: Option<BulletScope>) -> Result<Vec<PlaybookBulletRecord>> {
        let sql = match scope {
            Some(_) => {
                "SELECT id, scope, scope_key, category, text, bullet_type, state, maturity,
                 ...
                 WHERE state = 'active' AND scope = ?1
        ...
        let map_row = |row: &rusqlite::Row<'_>| -> rusqlite::Result<PlaybookBulletRecord> {
            ...
            Ok(PlaybookBulletRecord {
                ...
                helpful_count,
                harmful_count,
                feedback_events: Vec::new(), // loaded separately if needed
        ...
        let rows = match scope {
            Some(s) => stmt.query_map(params![scope_str(s)], map_row)?,
    ...
    pub fn get_playbook_bullet(&self, bullet_id: &BulletId) -> Result<Option<PlaybookBulletRecord>> {
        let bullets = self.list_active_bullets(None)?;
        Ok(bullets.into_iter().find(|b| b.id == *bullet_id))
    }
