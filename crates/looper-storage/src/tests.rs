...
fn insert_project_and_loop(repos: &Repositories) -> (String, String) {
    ...
    repos.projects.upsert(&ProjectRecord {
        id: pid.clone(), name: "test".into(), repo_path: "/tmp/r".into(),
        base_branch: None, archived: false, metadata_json: None,
        created_at: t.clone(), updated_at: t.clone(),
    }).unwrap();
    repos.loops.upsert(&LoopRecord {
        id: lid.clone(), seq: 1, project_id: pid.clone(),
        r#type: "worker".into(), target_type: "issue".into(),
        target_id: None, repo: None, pr_number: None, status: "idle".into(),
        config_json: None, metadata_json: None, last_run_at: None,
        next_run_at: None, created_at: t.clone(), updated_at: t,
    }).unwrap();
    (pid, lid)
}
...
fn make_queue_item(id: &str, r#type: &str, dedupe_key: &str) -> QueueItemRecord {
    let t = now();
    QueueItemRecord {
        id: id.into(), project_id: None, loop_id: None,
        r#type: r#type.into(), target_type: "loop".into(), target_id: "l-1".into(),
        repo: Some("org/repo".into()), pr_number: None,
        dedupe_key: dedupe_key.into(), priority: 1, status: "queued".into(),
        available_at: t.clone(), attempts: 0, max_attempts: 3,
        claimed_by: None, claimed_at: None, started_at: None, finished_at: None,
        lock_key: None, payload_json: None, last_error: None, last_error_kind: None,
        created_at: t.clone(), updated_at: t,
    }
}
...
fn test_projects_upsert_and_get() {
    ...
    repos.projects.upsert(&ProjectRecord {
        id: "p-1".into(), name: "test-project".into(), repo_path: "/tmp/repo".into(),
        base_branch: Some("main".into()), archived: false, metadata_json: None,
...
fn test_projects_list() {
    ...
    for i in 0..3 {
        repos.projects.upsert(&ProjectRecord {
            id: format!("p-{i}"), name: format!("proj-{i}"), repo_path: "/tmp/x".into(),
            base_branch: None, archived: false, metadata_json: None,
            created_at: t.clone(), updated_at: t.clone(),
        }).unwrap();
    }
...
fn test_projects_archive() {
    ...
    let t = now();
    repos.projects.upsert(&ProjectRecord {
        id: "p-1".into(), name: "x".into(), repo_path: "/tmp/x".into(),
        base_branch: None, archived: false, metadata_json: None,
        created_at: t.clone(), updated_at: t.clone(),
    }).unwrap();
...
fn test_loops_upsert_and_get() {
    ...
    let t = now();
    repos.loops.upsert(&LoopRecord {
        id: "loop-42".into(), seq: 10, project_id: pid,
        r#type: "worker".into(), target_type: "issue".into(),
        target_id: Some("#42".into()), repo: Some("org/repo".into()),
        pr_number: None, status: "idle".into(), config_json: None,
        metadata_json: None, last_run_at: None, next_run_at: None,
        created_at: t.clone(), updated_at: t,
    }).unwrap();
...
fn test_loops_get_by_seq() {
    ...
    let (pid, _) = insert_project_and_loop(&repos);
    repos.loops.upsert(&LoopRecord {
        id: "l-42".into(), seq: 42, project_id: pid,
        r#type: "worker".into(), target_type: "issue".into(),
        target_id: None, repo: None, pr_number: None, status: "running".into(),
        config_json: None, metadata_json: None, last_run_at: None,
...
fn test_loops_list_by_statuses() {
    ...
    for s in ["running", "paused"] {
        repos.loops.upsert(&LoopRecord {
            id: format!("l-{s}"), seq: 0, project_id: pid.clone(),
            r#type: "worker".into(), target_type: "issue".into(),
            target_id: None, repo: None, pr_number: None, status: s.into(),
            config_json: None, metadata_json: None, last_run_at: None,
            next_run_at: None, created_at: t.clone(), updated_at: t.clone(),
        }).unwrap();
    }
    // Should find l-1 (idle) + l-paused = 2
    assert_eq!(
        repos.loops.list_by_statuses(&["idle".into(), "paused".into()]).unwrap().len(),
        2
    );
}
...
fn test_loops_list_by_ids() {
    ...
    for i in 0..3 {
        repos.loops.upsert(&LoopRecord {
            id: format!("l-{i}"), seq: i, project_id: pid.clone(),
            r#type: "worker".into(), target_type: "issue".into(),
            target_id: None, repo: None, pr_number: None, status: "idle".into(),
            config_json: None, metadata_json: None, last_run_at: None,
            next_run_at: None, created_at: t.clone(), updated_at: t.clone(),
...
fn test_loops_terminate_by_project() {
    ...
    let t = now();
    repos.loops.upsert(&LoopRecord {
        id: "l-active".into(), seq: 2, project_id: pid.clone(),
        r#type: "worker".into(), target_type: "issue".into(),
        target_id: None, repo: None, pr_number: None, status: "idle".into(),
        config_json: None, metadata_json: None, last_run_at: None,
        next_run_at: None, created_at: t.clone(), updated_at: t.clone(),
    ...
    repos.loops.upsert(&LoopRecord {
        id: "l-done".into(), seq: 3, project_id: pid.clone(),
        r#type: "worker".into(), target_type: "issue".into(),
        target_id: None, repo: None, pr_number: None, status: "completed".into(),
        config_json: None, metadata_json: None, last_run_at: None,
        next_run_at: None, created_at: t.clone(), updated_at: t,
    }).unwrap();
...
fn test_loops_count_by_type_and_status() {
    ...
    repos.loops.upsert(&LoopRecord {
        id: "l-count-running".into(), seq: 10, project_id: pid.clone(),
        r#type: "worker".into(), target_type: "issue".into(),
        target_id: None, repo: None, pr_number: None, status: "running".into(),
        config_json: None, metadata_json: None, last_run_at: None,
        next_run_at: None, created_at: t.clone(), updated_at: t.clone(),
    ...
    repos.loops.upsert(&LoopRecord {
        id: "l-count-idle".into(), seq: 11, project_id: pid,
        r#type: "worker".into(), target_type: "issue".into(),
        target_id: None, repo: None, pr_number: None, status: "idle".into(),
        config_json: None, metadata_json: None, last_run_at: None,
        next_run_at: None, created_at: t.clone(), updated_at: t,
...
fn test_runs_upsert_and_get() {
    ...
    let t = now();
    repos.runs.upsert(&RunRecord {
        id: "run-1".into(), loop_id: lid, status: "running".into(),
        current_step: Some("plan".into()), last_completed_step: None,
        checkpoint_json: None, summary: None, error_message: None,
        started_at: t.clone(), last_heartbeat_at: Some(t.clone()),
        ended_at: None, created_at: t.clone(), updated_at: t,
    }).unwrap();
...
fn test_runs_get_latest_by_loop_id() {
    ...
    repos.runs.upsert(&RunRecord {
        id: "run-old".into(), loop_id: lid.clone(), status: "completed".into(),
        current_step: None, last_completed_step: None, checkpoint_json: None,
        summary: None, error_message: None, started_at: older.clone(),
        last_heartbeat_at: None, ended_at: Some(older.clone()),
        created_at: older, updated_at: t.clone(),
    }).unwrap();
    repos.runs.upsert(&RunRecord {
        id: "run-new".into(), loop_id: lid, status: "running".into(),
        current_step: Some("execute".into()), last_completed_step: None,
        checkpoint_json: None, summary: None, error_message: None,
        started_at: t.clone(), last_heartbeat_at: Some(t.clone()),
        ended_at: None, created_at: t.clone(), updated_at: t,
    }).unwrap();
...
fn test_runs_has_running() {
    ...
    let t = now();
    repos.runs.upsert(&RunRecord {
        id: "r1".into(), loop_id: lid, status: "running".into(),
        current_step: None, last_completed_step: None, checkpoint_json: None,
        summary: None, error_message: None, started_at: t.clone(),
        last_heartbeat_at: None, ended_at: None, created_at: t.clone(),
        updated_at: t,
    }).unwrap();
...
fn test_runs_list_by_loop() {
    ...
    for i in 0..3 {
        repos.runs.upsert(&RunRecord {
            id: format!("r-{i}"), loop_id: lid.clone(), status: "completed".into(),
            current_step: None, last_completed_step: None, checkpoint_json: None,
            summary: None, error_message: None, started_at: hours_ago(i),
            last_heartbeat_at: None, ended_at: Some(now()),
            created_at: hours_ago(i), updated_at: now(),
        }).unwrap();
    }
...
fn test_runs_list_by_status() {
    ...
    let t = now();
    repos.runs.upsert(&RunRecord {
        id: "r1".into(), loop_id: lid, status: "failed".into(),
        current_step: None, last_completed_step: None, checkpoint_json: None,
        summary: None, error_message: Some("oops".into()),
        started_at: t.clone(), last_heartbeat_at: None,
        ended_at: Some(t.clone()), created_at: t.clone(), updated_at: t,
    }).unwrap();
...
fn test_runs_count_by_status() {
    ...
    for s in ["running", "completed"] {
        repos.runs.upsert(&RunRecord {
            ...
            loop_id: lid.clone(), status: s.into(), current_step: None,
            last_completed_step: None, checkpoint_json: None, summary: None,
            error_message: None, started_at: t.clone(), last_heartbeat_at: None,
...
fn test_queue_create_or_get_active_by_dedupe_inserts() {
    let (repos, _dir) = setup();
    let (created, is_new) = repos.queue.create_or_get_active_by_dedupe(
        &make_queue_item("q-1", "planner", "unique-key")
    ).unwrap();
    assert!(is_new);
...
fn test_queue_mark_retry() {
    let (repos, _dir) = setup();
    repos.queue.upsert(&make_queue_item("q-1", "planner", "dp-1")).unwrap();
    repos.queue.mark_retry(&QueueMarkRetryInput {
        id: "q-1".into(), available_at: now(), attempts: 1,
        error_message: Some("transient error".into()),
        error_kind: "retryable_transient".into(), updated_at: now(),
    }).unwrap();
    let got = repos.queue.get_by_id("q-1").unwrap().unwrap();
    assert_eq!(got.status, "queued");
...
fn test_queue_fail() {
    let (repos, _dir) = setup();
    repos.queue.upsert(&make_queue_item("q-1", "planner", "dp-1")).unwrap();
    repos.queue.fail(&QueueFailInput {
        id: "q-1".into(), attempts: 2, finished_at: now(),
        error_message: Some("fatal".into()),
        error_kind: "non_retryable".into(), updated_at: now(),
    }).unwrap();
...
fn test_queue_cancel_by_loop() {
    let (repos, _dir) = setup();
    let (_, lid) = insert_project_and_loop(&repos);
    repos.queue.upsert(&QueueItemRecord {
        project_id: None, loop_id: Some(lid.clone()), ..make_queue_item("q-1", "planner", "dp-1")
    }).unwrap();
...
fn test_queue_list_queued_with_limit() {
    ...
    for i in 0..5 {
        repos.queue.upsert(
            &make_queue_item(&format!("q-{i}"), "planner", &format!("dp-{i}"))
        ).unwrap();
    }
...
fn test_queue_count_active_by_loop_id() {
    ...
    let (_, lid) = insert_project_and_loop(&repos);
    repos.queue.upsert(&QueueItemRecord {
        loop_id: Some(lid.clone()), ..make_queue_item("q-1", "planner", "dp-1")
    }).unwrap();
    repos.queue.upsert(&QueueItemRecord {
        loop_id: Some(lid), ..make_queue_item("q-2", "reviewer", "dp-2")
    }).unwrap();
...
fn test_queue_cancel_by_project() {
    ...
    let (pid, _) = insert_project_and_loop(&repos);
    repos.queue.upsert(&QueueItemRecord {
        project_id: Some(pid), loop_id: None, ..make_queue_item("q-1", "planner", "dp-1")
    }).unwrap();
...
fn test_locks_acquire_and_get() {
    let (repos, _dir) = setup();
    let lock = LockRecord {
        key: "lk-1".into(), owner: "w".into(), reason: None,
        expires_at: hours_ago(-1), created_at: now(), updated_at: now(),
    };
...
fn test_locks_acquire_conflict() {
    ...
    let future_lock = LockRecord {
        key: "lk-1".into(), owner: "w1".into(), reason: None,
        expires_at: hours_ago(-1), created_at: now(), updated_at: now(),
    };
    ...
    let past_lock = LockRecord {
        key: "lk-1".into(), owner: "w2".into(), reason: None,
        expires_at: hours_ago(1), created_at: now(), updated_at: now(),
    };
...
fn test_locks_acquire_expired_can_be_replaced() {
    let (repos, _dir) = setup();
    repos.locks.acquire(&LockRecord {
        key: "lk-y".into(), owner: "stale".into(), reason: None,
        expires_at: hours_ago(2), created_at: now(), updated_at: now(),
    }).unwrap();
    assert!(repos.locks.acquire(&LockRecord {
        key: "lk-y".into(), owner: "new-owner".into(), reason: None,
        expires_at: hours_ago(-1), created_at: now(), updated_at: now(),
    }).unwrap());
...
fn test_locks_refresh() {
    ...
    assert!(repos.locks.refresh(&LockRecord {
        key: "lk-1".into(), owner: "w".into(), reason: Some("still working".into()),
        expires_at: hours_ago(-2), created_at: now(), updated_at: now(),
    }).unwrap());
}
...
fn test_locks_list_expired() {
    let (repos, _dir) = setup();
    repos.locks.acquire(&LockRecord {
        key: "expired".into(), owner: "w".into(), reason: None,
        expires_at: hours_ago(1), created_at: now(), updated_at: now(),
    }).unwrap();
    repos.locks.acquire(&LockRecord {
        key: "active".into(), owner: "w".into(), reason: None,
        expires_at: hours_ago(-1), created_at: now(), updated_at: now(),
    }).unwrap();
...
fn test_events_append_and_list() {
    let (repos, _dir) = setup();
    repos.events.append(&EventLogRecord {
        id: "evt-1".into(), event_type: "test.event".into(),
        project_id: None, loop_id: None, run_id: None,
        entity_type: None, entity_id: None, correlation_id: None,
        causation_id: None, actor_type: None, actor_id: None,
        actor_display_name: None, payload_json: "{}".into(), created_at: now(),
    }).unwrap();
...
fn test_events_list_since() {
    ...
    let old = hours_ago(3);
    repos.events.append(&EventLogRecord {
        id: "evt-old".into(), event_type: "old".into(),
        project_id: None, loop_id: None, run_id: None,
        entity_type: None, entity_id: None, correlation_id: None,
        causation_id: None, actor_type: None, actor_id: None,
        actor_display_name: None, payload_json: "{}".into(), created_at: old,
    }).unwrap();
    repos.events.append(&EventLogRecord {
        id: "evt-new".into(), event_type: "new".into(),
        project_id: None, loop_id: None, run_id: None,
        entity_type: None, entity_id: None, correlation_id: None,
        causation_id: None, actor_type: None, actor_id: None,
        actor_display_name: None, payload_json: "{}".into(), created_at: now(),
...
fn test_events_list_by_entity() {
    let (repos, _dir) = setup();
    repos.events.append(&EventLogRecord {
        id: "evt-1".into(), event_type: "created".into(),
        project_id: None, loop_id: None, run_id: None,
        entity_type: Some("project".into()), entity_id: Some("p-1".into()),
        correlation_id: None, causation_id: None, actor_type: None,
        actor_id: None, actor_display_name: None,
        payload_json: "{}".into(), created_at: now(),
    }).unwrap();
...
fn test_eventlog_emit() {
    ...
    let record = log.emit(&AppendInput {
        event_type: "test.event".into(), project_id: None,
        ..AppendInput::new("")
    }).unwrap();
...
fn test_eventlog_emit_with_explicit_id() {
    ...
    let log = crate::EventLog::new(repos.events);
    let record = log.emit(&AppendInput {
        event_type: "custom.event".into(), id: Some("my-custom-id".into()),
...
fn test_notifications_upsert_and_get() {
    let (repos, _dir) = setup();
    let t = now();
    repos.notifications.upsert(&NotificationRecord {
        id: "n-1".into(), project_id: None, loop_id: None, run_id: None,
        entity_type: None, entity_id: None, channel: "log".into(),
        level: "info".into(), title: "Test".into(), subtitle: None,
        body: "Body".into(), status: "pending".into(),
        dedupe_key: Some("dk-1".into()), error_message: None,
        payload_json: None, sent_at: None,
        created_at: t.clone(), updated_at: t,
    }).unwrap();
...
fn test_notifications_list() {
    ...
    for i in 0..3 {
        repos.notifications.upsert(&NotificationRecord {
            id: format!("n-{i}"), project_id: None, loop_id: None, run_id: None,
            entity_type: None, entity_id: None, channel: "log".into(),
            level: "info".into(), title: format!("notif {i}"), subtitle: None,
            body: "".into(), status: "pending".into(), dedupe_key: None,
            error_message: None, payload_json: None, sent_at: None,
            created_at: t.clone(), updated_at: t.clone(),
        }).unwrap();
    }
...
fn test_notifications_get_latest_by_dedupe() {
    ...
    repos.notifications.upsert(&NotificationRecord {
        id: "n-1".into(), project_id: None, loop_id: None, run_id: None,
        entity_type: None, entity_id: None, channel: "log".into(),
        level: "info".into(), title: "first".into(), subtitle: None,
        body: "".into(), status: "sent".into(),
        dedupe_key: Some("dk-test".into()), error_message: None,
        payload_json: None, sent_at: Some(hours_ago(2)),
        created_at: hours_ago(2), updated_at: hours_ago(2),
    }).unwrap();
    repos.notifications.upsert(&NotificationRecord {
        id: "n-2".into(), project_id: None, loop_id: None, run_id: None,
        entity_type: None, entity_id: None, channel: "log".into(),
        level: "info".into(), title: "second".into(), subtitle: None,
        body: "".into(), status: "pending".into(),
        dedupe_key: Some("dk-test".into()), error_message: None,
        payload_json: None, sent_at: None,
        created_at: t.clone(), updated_at: t,
    }).unwrap();
...
fn test_worktrees_upsert_and_get() {
    let (repos, _dir) = setup();
    let (pid, _) = insert_project_and_loop(&repos);
    repos.worktrees.upsert(&WorktreeRecord {
        id: "wt-1".into(), project_id: pid, repo_path: "/tmp/r".into(),
        worktree_path: "/tmp/r-wt".into(), branch: "feature-x".into(),
        base_branch: Some("main".into()), status: "active".into(),
        head_sha: Some("abc123".into()), metadata_json: None,
        created_at: now(), updated_at: now(), cleaned_at: None,
    }).unwrap();
...
fn test_worktrees_get_by_branch() {
    ...
    let (pid, _) = insert_project_and_loop(&repos);
    repos.worktrees.upsert(&WorktreeRecord {
        id: "wt-1".into(), project_id: pid.clone(), repo_path: "/tmp/r".into(),
        worktree_path: "/tmp/r-wt".into(), branch: "my-branch".into(),
        base_branch: None, status: "active".into(), head_sha: None,
...
fn test_worktrees_list_by_project() {
    ...
    for i in 0..2 {
        repos.worktrees.upsert(&WorktreeRecord {
            id: format!("wt-{i}"), project_id: pid.clone(),
            repo_path: "/tmp/r".into(), worktree_path: format!("/tmp/r-{i}"),
            branch: format!("br-{i}"), base_branch: None, status: "active".into(),
            head_sha: None, metadata_json: None,
            created_at: now(), updated_at: now(), cleaned_at: None,
        }).unwrap();
    }
...
fn test_worktrees_list_cleanup_candidates() {
    ...
    let (pid, _) = insert_project_and_loop(&repos);
    repos.worktrees.upsert(&WorktreeRecord {
        id: "wt-cleanable".into(), project_id: pid.clone(),
        repo_path: "/tmp/r".into(), worktree_path: "/tmp/r-clean".into(),
        branch: "done-br".into(), base_branch: None, status: "abandoned".into(),
        head_sha: None, metadata_json: None,
        created_at: hours_ago(5), updated_at: hours_ago(5), cleaned_at: None,
    }).unwrap();
    repos.worktrees.upsert(&WorktreeRecord {
        id: "wt-active".into(), project_id: pid, repo_path: "/tmp/r".into(),
        worktree_path: "/tmp/r-active".into(), branch: "active-br".into(),
        base_branch: None, status: "active".into(), head_sha: None,
        metadata_json: None, created_at: now(), updated_at: now(),
        cleaned_at: None,
    }).unwrap();
...
fn test_worktrees_list_active() {
    ...
    repos.worktrees.upsert(&WorktreeRecord {
        id: "wt-1".into(), project_id: pid.clone(), repo_path: "/tmp/r".into(),
        worktree_path: "/tmp/r-1".into(), branch: "active".into(),
        base_branch: None, status: "active".into(), head_sha: None,
        metadata_json: None, created_at: now(), updated_at: now(),
        cleaned_at: None,
    }).unwrap();
    repos.worktrees.upsert(&WorktreeRecord {
        id: "wt-2".into(), project_id: pid, repo_path: "/tmp/r".into(),
        worktree_path: "/tmp/r-2".into(), branch: "stale".into(),
        base_branch: None, status: "abandoned".into(), head_sha: None,
        metadata_json: None, created_at: now(), updated_at: now(),
        cleaned_at: None,
    }).unwrap();
...
fn test_worktrees_touch_cleanup_attempt() {
    ...
    repos.worktrees.upsert(&WorktreeRecord {
        id: "wt-1".into(), project_id: pid, repo_path: "/tmp/r".into(),
        worktree_path: "/tmp/r-wt".into(), branch: "br".into(),
        base_branch: None, status: "abandoned".into(), head_sha: None,
        metadata_json: None, created_at: now(), updated_at: now(),
        cleaned_at: None,
    }).unwrap();
...
fn test_agent_executions_upsert_and_get() {
    let (repos, _dir) = setup();
    let t = now();
    repos.agent_executions.upsert(&AgentExecutionRecord {
        id: "ae-1".into(), project_id: None, loop_id: None, run_id: None,
        vendor: "claude".into(), status: "running".into(), pid: Some(12345),
        command_json: Some(r#"["arg1"]"#.into()), cwd: Some("/tmp".into()),
        summary: None, parse_status: None, completion_signal: None,
        heartbeat_count: 0, last_heartbeat_at: Some(t.clone()),
        output_json: None, error_message: None, native_session_id: None,
        native_resume_mode: None, native_resume_status: None,
        native_resume_error: None, started_at: t.clone(), ended_at: None,
        metadata_json: None, created_at: t.clone(), updated_at: t,
    }).unwrap();
...
fn test_agent_executions_get_latest_by_run_id() {
    ...
    repos.runs.upsert(&RunRecord {
        id: "r-1".into(), loop_id: lid, status: "running".into(),
        current_step: None, last_completed_step: None, checkpoint_json: None,
        summary: None, error_message: None, started_at: now(),
        last_heartbeat_at: None, ended_at: None, created_at: now(),
        updated_at: now(),
    }).unwrap();
    let t = now();
    repos.agent_executions.upsert(&AgentExecutionRecord {
        id: "ae-1".into(), project_id: None, loop_id: None,
        run_id: Some("r-1".into()), vendor: "codex".into(),
        status: "completed".into(), pid: None, command_json: None, cwd: None,
        summary: Some("done".into()), parse_status: None,
        completion_signal: None, heartbeat_count: 0, last_heartbeat_at: None,
        output_json: None, error_message: None, native_session_id: None,
        native_resume_mode: None, native_resume_status: None,
        native_resume_error: None, started_at: hours_ago(1),
        ended_at: Some(t.clone()), metadata_json: None,
        created_at: hours_ago(1), updated_at: t.clone(),
    }).unwrap();
    repos.agent_executions.upsert(&AgentExecutionRecord {
        id: "ae-2".into(), project_id: None, loop_id: None,
        run_id: Some("r-1".into()), vendor: "opencode".into(),
        status: "running".into(), pid: Some(9999), command_json: None,
        cwd: None, summary: None, parse_status: None,
        completion_signal: None, heartbeat_count: 0,
        last_heartbeat_at: Some(t.clone()), output_json: None,
        error_message: None, native_session_id: None,
        native_resume_mode: None, native_resume_status: None,
        native_resume_error: None, started_at: t.clone(), ended_at: None,
        metadata_json: None, created_at: t.clone(), updated_at: t,
    }).unwrap();
...
fn test_agent_executions_get_latest_active_by_run_id() {
    ...
    repos.agent_executions.upsert(&AgentExecutionRecord {
        ...
        status: "completed".into(), pid: None, command_json: None, cwd: None,
        summary: None, parse_status: None, completion_signal: None,
        heartbeat_count: 0, last_heartbeat_at: None, output_json: None,
        error_message: None, native_session_id: None,
        native_resume_mode: None, native_resume_status: None,
        native_resume_error: None, started_at: hours_ago(1),
        ended_at: Some(t.clone()), metadata_json: None,
...
fn test_agent_executions_list_active() {
    ...
    let t = now();
    repos.agent_executions.upsert(&AgentExecutionRecord {
        id: "running".into(), project_id: None, loop_id: None, run_id: None,
        vendor: "claude".into(), status: "running".into(), pid: Some(1),
        command_json: None, cwd: None, summary: None, parse_status: None,
        completion_signal: None, heartbeat_count: 0,
        last_heartbeat_at: Some(t.clone()), output_json: None,
        error_message: None, native_session_id: None,
        native_resume_mode: None, native_resume_status: None,
        native_resume_error: None, started_at: t.clone(), ended_at: None,
        metadata_json: None, created_at: t.clone(), updated_at: t.clone(),
    }).unwrap();
    repos.agent_executions.upsert(&AgentExecutionRecord {
        id: "completed".into(), project_id: None, loop_id: None, run_id: None,
        vendor: "claude".into(), status: "completed".into(), pid: None,
        command_json: None, cwd: None, summary: None, parse_status: None,
        completion_signal: None, heartbeat_count: 0, last_heartbeat_at: None,
        output_json: None, error_message: None, native_session_id: None,
        native_resume_mode: None, native_resume_status: None,
        native_resume_error: None, started_at: hours_ago(2),
        ended_at: Some(t.clone()), metadata_json: None,
        created_at: hours_ago(2), updated_at: t,
    }).unwrap();
...
fn test_pr_snapshots_upsert_and_list() {
    let (repos, _dir) = setup();
    let (pid, _) = insert_project_and_loop(&repos);
    repos.pull_request_snapshots.upsert(&PullRequestSnapshotRecord {
        id: "prs-1".into(), project_id: pid, repo: "org/repo".into(),
        pr_number: 42, head_sha: "abc".into(), base_sha: Some("def".into()),
        title: Some("My PR".into()), body: Some("Body".into()),
        author: Some("user1".into()), diff_ref: None, checks_summary: None,
        unresolved_thread_count: Some(0), review_state: Some("approved".into()),
        payload_json: None, captured_at: now(), created_at: now(),
    }).unwrap();
...
fn test_pr_snapshots_get_latest() {
    ...
    let t = now();
    repos.pull_request_snapshots.upsert(&PullRequestSnapshotRecord {
        id: "prs-old".into(), project_id: pid.clone(), repo: "org/repo".into(),
        pr_number: 42, head_sha: "old-sha".into(), base_sha: None,
        title: None, body: None, author: None, diff_ref: None,
        checks_summary: None, unresolved_thread_count: None, review_state: None,
        payload_json: None, captured_at: hours_ago(1), created_at: hours_ago(1),
    }).unwrap();
    repos.pull_request_snapshots.upsert(&PullRequestSnapshotRecord {
        id: "prs-new".into(), project_id: pid, repo: "org/repo".into(),
        pr_number: 42, head_sha: "new-sha".into(), base_sha: None,
        title: None, body: None, author: None, diff_ref: None,
        checks_summary: None, unresolved_thread_count: None, review_state: None,
        payload_json: None, captured_at: t.clone(), created_at: t,
    }).unwrap();
...
fn test_webhook_forwarders_upsert_list_delete() {
    ...
    repos.webhook_forwarders.upsert(&WebhookForwarderRecord {
        repo: "org/repo".into(), pid: 12345, process_start: t,
        fingerprint: "fp1".into(), endpoint: "http://localhost:9999".into(),
        events: "push,pull_request".into(), gh_path: "/usr/local/bin/gh".into(),
        daemon_id: "daemon-1".into(), spawned_at: t, updated_at: t,
    }).unwrap();
...
fn test_webhook_tunnel_hooks_upsert_get() {
    let (repos, _dir) = setup();
    let t = 1_700_000_000i64;
    repos.webhook_tunnel_hooks.upsert(&WebhookTunnelHookRecord {
        repo: "org/repo".into(), hook_id: 999,
        managed_url: "https://tunnel.example.com/hook".into(),
        secret_ref: "sec-1".into(), last_ping_at: None,
        consecutive_disables: 0, last_disable_at: None, orphaned: false,
...
fn test_webhook_tunnel_hooks_mark_orphaned() {
    ...
    let t = 1_700_000_000i64;
    repos.webhook_tunnel_hooks.upsert(&WebhookTunnelHookRecord {
        repo: "org/repo".into(), hook_id: 1, managed_url: "url".into(),
        secret_ref: "sec".into(), last_ping_at: None, consecutive_disables: 0,
        last_disable_at: None, orphaned: false, created_at: t, updated_at: t,
    }).unwrap();
