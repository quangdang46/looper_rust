...
fn test_daemon_starts_and_serves_health() {
    ...
    assert!(result.is_ok(), "daemon should become ready within 15s: {:?}", result.err());

    let body = result.unwrap();
    assert!(body.get("ok").and_then(|v| v.as_bool()).unwrap_or(false), "status response should have ok=true: {body}");
...
fn test_daemon_shows_version() {
    ...
    let resp = client.get(&version_url).send().expect("should get version endpoint");
    ...
    assert!(
        body.get("data").and_then(|d| d.get("version")).and_then(|v| v.as_str()).is_some(),
